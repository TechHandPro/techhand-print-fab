...
def test_discover_uses_lan_probe_and_skips_ssdp() -> None:
    ...
    assert result["printers"][0]["model"] == "BL-P001"
    assert result["printers"][0]["state"] == ""
    assert result["printers"][0]["ams"] is None
...
def test_discover_attaches_state_and_ams_from_a_read_only_lookup() -> None:
    def lookup(config: object, host: str, serial: str) -> dict[str, object]:
        assert host == "192.168.1.50"
        assert serial == "00M09A123456789"
        assert config.access_code == "99887766"  # type: ignore[attr-defined]
        return {"state": "IDLE", "ams": {"present": True, "trays": [{"slot": "0", "type": "PETG"}]}}
    ...
    assert printer["model"] == "BL-P001"
    assert printer["state"] == "IDLE"
    assert printer["ams"]["present"] is True
    assert result["printer_dispatched"] is False


def test_status_lookup_failure_keeps_the_printer() -> None:
    def lookup(_config: object, _host: str, _serial: str) -> dict[str, object]:
        raise BambuError("status timeout")
    ...
    assert printer["status_error"] == "status timeout"
    assert result["printer_dispatched"] is False
